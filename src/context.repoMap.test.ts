...
const ISOLATED_BUN_SCRIPT_TIMEOUT_MS = 60_000

afterEach(async () => {
  const { getRepoMapContext, getSystemContext } = await import('./context.js')
  const { clearParserCaches } = await import('./context/repoMap/parser.js')
  const { clearSymbolExtractorCaches } = await import('./context/repoMap/symbolExtractor.js')
...
describe('getRepoMapContext', () => {
  ...
  test('timeout cancellation is observed by the repo map build hook', async () => {
    ...
    const script = [
      "const { runRepoMapBuildWithTimeout } = await import('./src/context.ts')",
      ...
      '  },',
      '})',
      'const cancellation = await cancellationObserved',
      'if (result.result !== null || result.timedOut !== true || cancellation !== true) {',
      '  console.error(JSON.stringify({ result, cancellation }))',
      '  process.exit(1)',
      '}',
      'console.log(JSON.stringify({ timedOut: result.timedOut, cancellation }))',
    ].join('\n')
    const result = spawnSync(process.execPath, ['--eval', script], {
      cwd: process.cwd(),
      encoding: 'utf8',
      env: { ...process.env, FORCE_COLOR: '0' },
      timeout: ISOLATED_BUN_SCRIPT_TIMEOUT_MS,
    })

    if (result.error) throw result.error
    expect(result.stderr).toBe('')
    expect(result.status).toBe(0)
    expect(result.stdout.trim()).toBe('{"timedOut":true,"cancellation":true}')
  })
