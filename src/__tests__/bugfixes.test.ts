...
describe('Regression checks', () => {
  test('duplicate plugin hooks are deduplicated before execution', async () => {
    const previousConfigDir = process.env.OPENCLAUDE_CONFIG_DIR
    const isolatedConfigDir = mkdtempSync(join(tmpdir(), 'openclaude-hooks-'))
    process.env.OPENCLAUDE_CONFIG_DIR = isolatedConfigDir
    ...
    try {
      ...
      if (previousConfigDir === undefined) {
        ...
        process.env.OPENCLAUDE_CONFIG_DIR = previousConfigDir
      }
      resetHooksConfigSnapshot()
      resetSettingsCache()
