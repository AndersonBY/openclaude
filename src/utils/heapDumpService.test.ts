...
test('effective dump numbers are used in generated heap dump filenames', () => {
  ...
  assert.deepEqual(
    ...
    {
      heapPath: join('/tmp/dumps', 'session-123-dump1.heapsnapshot'),
      diagPath: join('/tmp/dumps', 'session-123-dump1-diagnostics.json'),
    },
  )
  assert.deepEqual(
    getHeapDumpFilePaths('session-123', '/tmp/dumps', secondManual),
    {
      heapPath: join('/tmp/dumps', 'session-123-dump2.heapsnapshot'),
      diagPath: join('/tmp/dumps', 'session-123-dump2-diagnostics.json'),
    },
  )
  assert.deepEqual(getHeapDumpFilePaths('session-123', '/tmp/dumps', 7), {
    heapPath: join('/tmp/dumps', 'session-123-dump7.heapsnapshot'),
    diagPath: join('/tmp/dumps', 'session-123-dump7-diagnostics.json'),
  })
})
