...
function setupConsumerProject(name: string): string {
  ...
  // Simulate node_modules/@makerbi/openclaude structure
  const pkgDir = join(tmpDir, 'node_modules', '@makerbi', 'openclaude')
...
describe('package consumer types', () => {
  ...
  test('SDKMessage/SDKUserMessage/SDKResultMessage are re-exported correctly', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKMessage, SDKUserMessage, SDKResultMessage } from '@makerbi/openclaude/sdk'`,
        ``,
  ...
  test('result usage standard counters are required numbers', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKResultMessage } from '@makerbi/openclaude/sdk'`,
        ``,
  ...
  test('SDKRateLimitError has resetsAt and rateLimitType as class properties', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import { SDKRateLimitError } from '@makerbi/openclaude/sdk'`,
        ``,
  ...
  test('control initialize response accepts generated model and account values', () => {
    ...
    writeFileSync(
      join(tmpDir, 'consumer.ts'),
      [
        `import type { SDKControlInitializeResponse, ModelInfo } from '@makerbi/openclaude/sdk'`,
        ``,
